...
class ProximitySensorManager {
     ...
     * <p>
     * This interface abstracts two possible states for the proximity sensor,
     * near and far.
     * <p>
    ...
    public interface Listener {
        /**
         * Called when the proximity sensor transitions from the far to the near
         * state.
         */
        public void onNear();

        /**
         * Called when the proximity sensor transitions from the near to the far
         * state.
         */
        public void onFar();
    }
    ...
    /**
     * The current state of the manager, i.e., whether it is currently tracking
     * the state of the
     * sensor.
     */
     ...
     * <p>
     * After creating an instance of this object, one should call
     * {@link #register()} and {@link #unregister()} to enable and disable the
     * notifications.
     * <p>
     * Instead of calling unregister, one can call {@link #unregisterWhenFar()}
     * to unregister the listener the next time the sensor reaches the
     * {@link State#FAR} state if currently in the {@link State#NEAR} state.
     */
    private static class ProximitySensorEventListener implements SensorEventListener {
         ...
         * <p>
         * Before registering and after unregistering we are always in the
         * {@link State#FAR} state.
         */
        private State mLastState;
        /**
         * If this flag is set to true, we are waiting to reach the
         * {@link State#FAR} state and
        ...
        /**
         * Unregister the next time the sensor reaches the {@link State#FAR}
         * state.
         */
     ...
     * <p>
     * The listener will stop receiving notifications of events, possibly after
     * receiving a last {@link Listener#onFar()} callback.
     * <p>
     * If {@code waitForFarState} is true, if the sensor is not currently in the
     * {@link State#FAR} state, the listener will receive a
     * {@link Listener#onFar()} callback the next time the sensor actually
     * reaches the {@link State#FAR} state.
     * <p>
     * If {@code waitForFarState} is false, the listener will receive a
     * {@link Listener#onFar()} callback immediately if the sensor is currently
     * not in the {@link State#FAR} state.
     * <p>
     * This method is idempotent.
     */
